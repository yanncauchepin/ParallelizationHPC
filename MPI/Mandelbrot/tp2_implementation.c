...
int main(int argc, char *argv[])
{
  ...
  int nb_processus=p;
  int largeur_block,source,dest=0,tag=0;

  largeur_block = h/nb_processus;
...
//   fprintf( stderr, "Dim image: %dx%d\n", w, h);
  ...
  y = ymin + largeur_block*my_rank*yinc;
  ...
  else
  {
      ...
      if(nb_processus>1)
      {
        for(source=1;source<nb_processus;source++)
        {
        
            MPI_Recv(&image[largeur_block*w*source], sizeof(unsigned char)*w*largeur_block, MPI_UNSIGNED_CHAR, source, tag, MPI_COMM_WORLD,&status);
