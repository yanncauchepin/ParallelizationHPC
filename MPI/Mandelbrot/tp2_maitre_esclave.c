...
//   fprintf( stderr, "Dim image: %dx%d\n", w, h);
  ...
  if(my_rank!=0)
  {
    ...
    while(fini==0) 
	{
        MPI_Recv(&num_ligne, sizeof(int), MPI_INT, 0, 1, MPI_COMM_WORLD,&status);
        fprintf(stderr,"Test 4 processus %d %d\n",my_rank,num_ligne);
		
        y = ymin + num_ligne*yinc;
        ...
        MPI_Send(block,sizeof(unsigned char)*w,MPI_UNSIGNED_CHAR,0,num_ligne,MPI_COMM_WORLD);
        MPI_Recv(&fini, sizeof(int), MPI_INT, 0, 0, MPI_COMM_WORLD,&status);
  ...
  else
  {
    ...
    while(i<(nb_processus-1)&& (i<h))
    {	
	  MPI_Send(&fini,sizeof(int),MPI_INT,(i+1),0,MPI_COMM_WORLD);
		
        MPI_Send(&i,sizeof(int),MPI_INT,(i+1),1,MPI_COMM_WORLD);
        i++;
    }
	...
	while(fini==0)
	{
		...
		if(i<h)
		{
			...
			MPI_Send(&i,sizeof(int),MPI_INT,status.MPI_SOURCE,1,MPI_COMM_WORLD);
		}
		else
		{
			if(nb_reception==h)
